...
function createParticles(ctx, canvas, text, textX, textY, font, color, alignment) {
  ...
  // Optimize: Increase base sample rate on retina screens to lower particle overhead
  const baseSampleRate = Math.max(1.5, Math.round(currentDPR / 2.5));
...
function renderCanvas({ framerProps, canvasRef, wrapperSize, particlesRef, globalDpr, currentTextIndex, transformedDensity }) {
  ...
  const fontSize = parseInt((framerProps.font?.fontSize || "50px").replace("px", ""));
...
export default function VaporizeTextCycle({
  ...
  // Start/stop animation when entering/leaving view
  useEffect(() => {
    if (isInView) {
    ...
    const animate = (currentTime) => {
      ...
      switch (animationState) {
        ...
        case "fadingIn":
          ...
          ctx.save();
          ctx.scale(globalDpr, globalDpr);
          const fadeVal = Math.min(fadeOpacityRef.current, 1);
          particlesRef.current.forEach((particle) => {
            particle.x = particle.originalX;
            particle.y = particle.originalY;
            const opacity = fadeVal * particle.originalAlpha;
  ...
  // Render canvas when text/size changes
  useEffect(() => {
